...
function getUserPurchasedBooks($pdo, $userID) {
    $stmt = $pdo->prepare("
        ...
        WHERE 
            br.usID = (select usID from users where usUsername= :userID) AND br.transType = 'buy'
    ");
...
function getUserRentedBooks($pdo, $userID) {
    $stmt = $pdo->prepare("
        ...
        FROM 
            ...
            JOIN payment p ON br.payID = p.payID
        WHERE 
            br.usID = (select usID from users where usUsername= :userID) AND br.transType = 'rent'
    ");
    $stmt->execute(['userID' => $userID]);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
        ...
        <nav class="col-md-2 d-none d-md-block sidebar">
            <div class="sidebar-sticky">
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="usersprint.php">
