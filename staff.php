...
function getTotalRent($pdo) {
    $stmt = $pdo->query("SELECT SUM(p.payAmount) FROM buyrent b JOIN payment p ON b.payID = p.payID WHERE b.transType = 'rent' AND b.status = 'Approved'");
    return $stmt->fetchColumn() ?: 0;
}

// Function to get total bought
function getTotalBought($pdo) {
    $stmt = $pdo->query("SELECT SUM(p.payAmount) FROM buyrent b JOIN payment p ON b.payID = p.payID WHERE b.transType = 'buy' AND b.status = 'Approved'");
    return $stmt->fetchColumn() ?: 0;
}
            ...
            <div class="sidebar-sticky" >
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="empRegister.php">
                                <i class="zmdi zmdi-widgets"></i>
                                Manage Staff
                            </a>
                        </li>
